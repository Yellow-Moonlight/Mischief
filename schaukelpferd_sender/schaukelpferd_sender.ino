...
void setup() {
  ...
  mpu.setYAccelOffset(244);
  mpu.setZAccelOffset(1055);  // 1688 factory default for my test chip
  ...
  BLEDevice::init("ESP32_Sensor_1");  // Initialize BLE device
...
void loop() {
  ...
  if (mpu.dmpGetCurrentFIFOPacket(fifoBuffer)) {  // Get the latest packet
    ...
    val = ypr[1] * 180 / M_PI;  // Calculate pitch angle in degrees

    blinkState = !blinkState;
    digitalWrite(LED_PIN, blinkState);
  }

  int absval = int(abs(val));
  ...
  if (currentTime - lastTime >= 20) {
    ...
    String dataToSend = String(absval);  // Example data
